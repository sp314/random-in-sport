...
This process is marked by a few steps. In general we want to:
...
```{r info_content_exec, warning=FALSE}
...
nba_11_info_metrics <- nba_11_info_sets %>%
  ...
  mutate(
    ...
    sim = future_map(.x = pred_prob, .f = ~rbernoulli(n = 1, p = .x)),
...
nba_11_mov <- lm(formula = home_mov ~., data = nba_11_model_matrix)
summary(nba_11_mov)
```
...
```{r massey_plot}
nba_11_mov_thetas <- coef(summary(nba_11_mov)) %>%
  data.frame() %>%
  rownames_to_column(var = "team") %>%
