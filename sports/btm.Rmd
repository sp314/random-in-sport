...
btm <- BTm(outcome = home_win, #1 if player1 (home) wins and 0 if player2 (away) does
...
qvcalc(BTabilities(btm))$qvframe %>% #quasi normal standard errors
...
```{r bt_days}
# Fit Bradley-Terry rating model for binary outcomes
btm_days <- BTm(outcome = home_win, #1 if player1 (home) wins and 0 if player2 (away) does
...
qvcalc(BTabilities(btm_days))$qvframe %>% #quasi normal standard errors
  rownames_to_column(var = "team") %>%
  ggplot(aes(x = reorder(as.factor(team), estimate), color = as.factor(team))) +
  geom_point(aes(y = estimate)) +
  geom_errorbar(aes(ymin = estimate - quasiSE, ymax = estimate + quasiSE)) + 
...
btm_model_matrix <- nba_11 %>%
  ...
  rbind(c(1 , rep(1, length(levels(nba_11$home))), 0, 0)) # Arbitrary linear constraint for unique solution vec

btm_days_manual <- glm(formula = home_win ~ ., family = "binomial", data = btm_model_matrix)
summary(btm_days_manual)
```
...
Continuing onward, it is quite possible that there is a non-linear relationship between days out and winning odds, additionally the sample is centered about around 2 `days_since_last_game`. I want to test this first by fitting the same model as (2), having the number of days out be a factor variable. The equation is outlined below where 9 is the highest number of days out, $\delta_{h, i}$ is the effect of having a team be $i$ days out on the winning odds for the home team and $d_{h, i}$ is 1 or 0 whether the home team is $i$ days out or not.

\begin{equation}
  \displaystyle \ln\left( \frac{\pi_{i,j}}{1 - \pi_{i,j}} \right) = \theta_i - \theta_j + \alpha + \sum_{i = 1}^{9}\delta_{i}d_{a, i} + \sum_{i = 1}^{9}\delta_{i}d_{a, i}
\end{equation}

```{r bt_days_fct}
all_days_values <- unique(union(nba_11$home_days_since_last_game, nba_11$away_days_since_last_game))

nba_11_days_fct <- nba_11 %>%
  mutate(
...
btm_days <- BTm(outcome = home_win, #1 if player1 (home) wins and 0 if player2 (away) does
    player1 = data.frame(team = home, hfa = 1, days_out = home_days_since_last_game), 
    player2 = data.frame(team = away, hfa = 0, days_out = away_days_since_last_game),
    formula = ~team + hfa + days_out, id = "team", data = nba_11_days_fct)
...
```{r bt_distance}
# Fit Bradley-Terry rating model for binary outcomes
btm_distance <- BTm(outcome = home_win, #1 if player1 (home) wins and 0 if player2 (away) does
...
summary(btm_distance)
```

```{r bt_distance_plot, echo=FALSE}
qvcalc(BTabilities(btm_distance))$qvframe %>% #quasi normal standard errors
  rownames_to_column(var = "team") %>%
  ggplot(aes(x = reorder(as.factor(team), estimate), color = as.factor(team))) +
  geom_point(aes(y = estimate)) +
  geom_errorbar(aes(ymin = estimate - quasiSE, ymax = estimate + quasiSE)) + 
...
info_sets <- training_split_df(df = nba_11, prop = c(0.125,0.25, 0.375, 0.50, 0.625, 0.75, 0.875), n_sets = 100)
