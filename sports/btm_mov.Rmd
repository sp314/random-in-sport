...
nba_10_btm <- BTm(outcome = home_win, #1 if player1 (home) wins and 0 if player2 (away) does
    player1 = data.frame(team = as.factor(home), hfa = 1, days_since_last_game = home_days_since_last_game), 
    player2 = data.frame(team = as.factor(away), hfa = 0, days_since_last_game = away_days_since_last_game),
...
nba_10_info_sets <- training_split_df(df = nba_10, prop = 0.125, n_sets = 20) %>%
  mutate(
    set_id = row_number(),
...
```{r}
#[1] 0.5769981
mean(nba_10_info_sets$info_metric)
```
